...
def l1loss(img1,img2,mask=1,eps=1e-2):
    ...
    sigma = torch.tensor([[1.0, 1.0]] * batch_size)
    img1 = KF.gaussian_blur2d(img1,[3,3],sigma)*mask_
    img2 = KF.gaussian_blur2d(img2,[3,3],sigma)*mask_
...
def smoothloss(disp,img=None):
    ...
    local_smooth_re = 0
    for d in smooth_d:
        sigma = torch.tensor([d // 6, d // 6], dtype=torch.float32).unsqueeze(0).repeat(disp.size(0), 1)
        local_mean = KF.gaussian_blur2d(disp,[d,d],sigma,border_type='replicate')
