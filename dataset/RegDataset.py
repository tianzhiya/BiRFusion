...
import cv2
import torch
from Tools.utils import randflow, randrot, randfilp
...
class CityScapes(Dataset):
    def __init__(
        ...
        with open('../Tools/cityscapes_info.json', 'r') as fr:
...
class RegData(torch.utils.data.Dataset):
    ...
    def imread(path, flags=cv2.IMREAD_GRAYSCALE):
        ...
        # im_ts = kornia.Tools.image_to_tensor(im_cv / 255.,keepdim=False).type(torch.FloatTensor)
